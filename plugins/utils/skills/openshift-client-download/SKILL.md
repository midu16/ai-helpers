...
description: OpenShift Client Download — select, verify, and install official CLI binaries (OpenShift oc/kubectl/openshift-install, ROSA, Helm, opm, operator-sdk, and common helpers) from user-described needs on Linux or macOS, using mirror and vendor sites with checksum verification when available
---

# OpenShift Client Download

Use the **OpenShift Client Download** skill when the user (or a slash command) asks to **download**, **install**, or **pin versions** of platform CLIs—especially OpenShift and cloud tooling—based on a **natural-language description** (for example: “oc and openshift-install for 4.16 on Linux arm64”, “latest stable ROSA CLI”, “helm 3 plus yq”).
...
- **Typical files** (names vary slightly by release):
  - Linux x86_64: `openshift-client-linux.tar.gz`, often `openshift-install-linux.tar.gz` in the same version folder for IPI/UPI installers.
  - Linux arm64: `openshift-client-linux-arm64.tar.gz` (and matching install archive if present).
...
- If the user’s description matches **internal** or **team-specific** bundles (for example a private script such as `00_client_download.sh` in an external repo), **merge** that script’s artifact list with this **OpenShift Client Download** catalog after the user pastes it or points to an accessible copy—do not invent private URLs.
